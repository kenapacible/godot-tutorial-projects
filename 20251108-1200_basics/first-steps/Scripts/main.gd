...
func _process(delta_seconds: float) -> void:
	...
	if $Sprite2D.position.x > screen_width or $Sprite2D.position.x < 0:
	...
	if $Sprite2D.position.y > screen_height or $Sprite2D.position.y < 0:
	...
	$Sprite2D.position.x += sprite_speed_x * delta_seconds;
	$Sprite2D.position.y += sprite_speed_y * delta_seconds;
